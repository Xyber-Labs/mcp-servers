...
from mcp_server_together_imgen.x402_integration.config import (
    PaymentOptionConfig,
    X402Config,
)
...
class TestToolPricingYamlValidation:
    ...
    def test_loads_valid_yaml_with_pricing_for_multiple_endpoints(
        self, tmp_path: Path
    ):
...
# =============================================================================
# Feature 7: TogetherSettings Configuration
# =============================================================================


class TestTogetherSettings:
    """Tests for TogetherSettings loading and defaults."""

    def test_default_models(self):
        """TogetherSettings has correct default models."""
        ...
        settings = TogetherSettings(api_key="test-key", _env_file=None)

        assert settings.default_model == "black-forest-labs/FLUX.1-dev"
        assert settings.lora_model == "black-forest-labs/FLUX.1-dev-lora"
        assert settings.refiner_model == "deepseek-ai/DeepSeek-V3"

    def test_default_timeout(self):
        """TogetherSettings has 5-minute default timeout for image generation."""
        from mcp_server_together_imgen.together.config import TogetherSettings

        settings = TogetherSettings(api_key="test-key", _env_file=None)

        assert settings.generation_timeout == 300  # 5 minutes

    def test_refinement_prompts_defined(self):
        """TogetherSettings has both standard and LoRA refinement prompts."""
        from mcp_server_together_imgen.together.config import TogetherSettings

        settings = TogetherSettings(api_key="test-key", _env_file=None)

        assert settings.refine_prompt_standard is not None
        assert len(settings.refine_prompt_standard) > 0
        assert settings.refine_prompt_lora is not None
        assert len(settings.refine_prompt_lora) > 0
        # They should be different
        assert settings.refine_prompt_standard != settings.refine_prompt_lora
