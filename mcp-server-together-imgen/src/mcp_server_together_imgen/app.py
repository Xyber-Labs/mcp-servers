...
from mcp_server_together_imgen.api_routers import routers as api_routers
from mcp_server_together_imgen.dependencies import DependencyContainer
...
from mcp_server_together_imgen.x402_integration.config import get_x402_settings
...
async def app_lifespan(app: FastAPI):
    ...
    together_settings = TogetherSettings()
    together_client = TogetherClient(together_settings)

    # Create container with dependencies
    DependencyContainer.create(together_client=together_client)
    ...
    # Add any cleanup if TogetherClient needs it
...
def create_app() -> FastAPI:
    ...
    # Note: MCP-only routers will be added here in Phase 5
    ...
    # --- Main Application ---
    app = FastAPI(
        title="Together Image Generation MCP Server (Hybrid)",
