...
from mcp_twitter.x402_integration import (
    CHAIN_ID_TO_NETWORK,
    EVM_NETWORKS,
    SOLANA_NETWORKS,
    get_x402_settings,
)
from mcp_twitter.x402_integration.accepted_assets import CUSTOM_NETWORKS
from mcp_twitter.x402_integration.config import PaymentOptionConfig, X402Config
            ...
            for opt in options:
                if network := CHAIN_ID_TO_NETWORK.get(opt.chain_id):
                    ...
                    logger.warning(
                        ...
                        f"Add it to CHAIN_ID_TO_NETWORK mapping in accepted_assets.py"
                    )
        ...
        for network in networks_used:
            if network in EVM_NETWORKS:
                ...
                logger.warning(
                    ...
                    f"Update accepted_assets.py to classify this network."
                )
