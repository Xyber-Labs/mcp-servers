...
def require_base_url(config: E2ETestConfig) -> None:
    if not config.base_url:
        ...
        pytest.skip("Set MCP_YOUTUBE_TEST_BASE_URL to run E2E tests.")
...
def require_wallet(config: E2ETestConfig) -> None:
    if not config.private_key:
        ...
        pytest.skip("Set MCP_YOUTUBE_TEST_PRIVATE_KEY to run x402 payment E2E tests.")


def require_apify_token(config: E2ETestConfig) -> str:
    if not config.apify_token:
        import pytest

        pytest.skip(
            "Set MCP_YOUTUBE_TEST_APIFY_TOKEN to run Apify integration E2E tests."
        )
